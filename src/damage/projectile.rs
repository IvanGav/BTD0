...
pub fn despawn_simple_projectile(mut cmd: Commands, p: Query<(Entity, &SimpleProjectile)>) {
    for (e, p) in &p {
        if p.lifetime <= 0 {
...
pub fn despawn_stray_projectile(mut cmd: Commands, p: Query<(Entity, &Transform), With<DamageDealer>>) {
    for (e, p) in &p {
        if p.translation.x > 500. || p.translation.x < -500. || p.translation.y > 500. || p.translation.y < -500. {
            cmd.entity(e).despawn();
        }
    }
}
